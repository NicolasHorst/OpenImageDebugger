...
class CodingStructure(interface.TypeInspectorInterface):
    """
    ...
    def get_buffer_metadata(self, obj_name, picked_obj, debugger_bridge):
        ...
        width = int(picked_obj['m_reco']['bufs']['_arr'][0]['width'])
        height= int(picked_obj['m_reco']['bufs']['_arr'][0]['height'])
        row_stride = int(picked_obj['m_reco']['bufs']['_arr'][0]['stride'])

        # remove const from type
        if 'const' in current_type:
            print("original current_type: %s" % current_type)
            current_type = current_type.split('const')[1].strip()
        ...
        buffer = debugger_bridge.get_casted_pointer(current_type,picked_obj['m_reco']['bufs']['_arr'][0]['buf'])
    ...
    def is_symbol_observable(self, symbol):
        ...
        type_regex = r'(const\s+)?CodingStructure(\s+?[*&])?'
